...
class ExpenseTracker {
    ...
    async addTransaction() {
        ...
        if (editingId) {
            ...
            if (this.isFirebaseUser && this.dataManager) {
                try {
                    await this.dataManager.updateTransaction(editingId, {
                        ...transactionData,
...
<body>
    ...
    async initFirebase() {
        try {
            ...
            const { default: FirebaseDataManager } = await import('./firebase-manager.js');
            this.dataManager = new FirebaseDataManager(this.userId);
            
            // Load data from Firebase
            this.transactions = await this.dataManager.getTransactions();
            this.receipts = await this.dataManager.getReceipts();
            
            // Set up real-time sync
            this.dataManager.setupRealtimeSync((dataType, data) => {
    ...
    async checkMigration() {
        ...
        if (hasLocalData && this.dataManager) {
            const migrate = confirm('We found local data. Would you like to migrate it to the secure cloud?');
            if (migrate) {
                this.showToast('Migrating data to cloud...', 'info');
                const success = await this.dataManager.migrateFromLocalStorage();
                if (success) {
                    ...
                    this.showToast('Migration failed. Using local data.', 'error');
                }
            }
        }
    }
    ...
    async saveData() {
        try {
            if (this.isFirebaseUser && this.dataManager) {
                // Data is automatically saved to Firebase through individual operations
                // No need to save all data at once as Firebase handles real-time sync
                return;
            } else {
